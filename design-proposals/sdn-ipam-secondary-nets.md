...
There are essentially three ways this feature can be developed:
- VM user creates the persistent IPs allocation
- KubeVirt creates the persistent IPs allocation
- the CNI plugin creates the persistent IPs allocation
...
requires KubeVirt to send all the required information to OVN-Kubernetes, and it
will create the persistent allocation, and manage its lifecycle. Take into
...
it to understand how to map IPAMClaims to the networks.

Below you can find an example of a network attachment definition, specifying
the behavior of attachments to the `tenantblue` network, along with an
`IPAMCLaim` for that network.
```yaml
---
