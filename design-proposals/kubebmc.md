...
No existing repos under the KubeVirt organization will be impacted. KubeBMC augments the KubeVirt ecosystem as an optional add-on providing an operator and the BMC implementation.

# Design

KubeBMC will act as a bridge between KubeVirt VMs and the BMC functionalities, using Kubernetes services to expose these capabilities. KubeBMC consists of two components:

- **kubebmc-controller**: A typical Kubernetes controller reconciles the KubeBMC, VirtualMachine, and Service objects
- **kbmc**: A BMC simulator for serving IPMI and Redfish endpoints and translating the incoming requests to native KubeVirt API calls

Each VirtualMachine will have its own KubeBMC object created by `kubebmc-controller` during the VM creation. The controller will further process the KubeBMC object to complete the BMC scaffold. This includes a BMC emulator Pod, i.e., `kbmc`, and its Service object working together to provide the BMC functionalities to various clients over the network.
...
The following graph illustrates the entire KubeBMC workflow:
...
It's also possible for VMs being created to opt out of such a functionality. This could be done by annotating the VirtualMachine object with a specific key-value pair.

## API Examples

The proposed KubeBMC CRD looks like the following:
...
KubeBMC is designed to scale within Kubernetes environments, and the number of emulated BMC Pods and Services should increase as the number of VM increases. Hence, the overhead each Pod introduced to the entire system should also be considered.
...
However, some chores need to be done before transferring. For example, the API group of the KubeBMC CRD needs to be replaced with a suitable name conforming to KubeVirt's convention. Also, the code needs to be protected by robust unit tests and integration test plans.
...
KubeBMC will support not only IPMI but also expose functionalities through the Redfish protocol. Additionally, it will enable virtual media operations for VMs, providing a new option for boot device selection.
