...
* As a cluster admin managing VM images, I want the ability to automate sourcing in operating system disk image updates and make them available to users.
...
* **SSP**: Manages setting up DataImportCrons objects with containerDisk image sources for commonly supported operating systems.
...
In order to leverage golden disk image auto delivery/updates at the HCO level, the HCO is responsible for configuring DataImportCrons on the SSP operator’s CR.

For instance, if the HCO wants to develop a feature for automatically sourcing in fedora golden images into a PVC to be used by the SSP's common templates, then the HCO needs to configure the location where the fedora image is sourced from on the SSP CR. 

This document doesn't include within its scope how the HCO should expose enabling the image auto delivery/update mechanism on the HCO API or where the containerDisk images are sourced.
...
status:
  lastImportedPVC: quay.io/kubevirt/fedora:33
```
