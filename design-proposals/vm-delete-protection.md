...
Affected VMs could be critical to application/s the cluster is running, or they may contain crucial data. Therefore, the
VM deletion protection aims to avoid accidental degradation of applications or loss of data.

## Goals

- Establish a delete protection mechanism for VirtualMachine objects.
...
Everyone who has the permissions to delete VMs.
...
`True`. If the annotation is present with one of the aforementioned values, requests to delete this VM will be rejected
by KubeVirt. By default, this annotation is not present on a VM. It is up to the user to enable the deletion protection.
The annotation looks like this in the VM manifests:
...
- Variables:
    - \`annotation\`: Returns the VM annotation field;
        - string('kubevirt.io/vm-delete-protection')
- Validation rules:
    - Checks if the annotation is present at all, if present, checks if it is equal to “true” or “True”;
        - (\!(variables.annotation in oldObject.metadata.annotations) || \!oldObject.metadata.annotations\[variables.annotation\].matches('^(true|True)$'))
...
#### Option 2\#: Virtual Machine Admission Webhook

A new validation rule is added to the VM admitters. The admitter will check for presence of
the \`[kubevirt.io/vm-delete-protection](http://kubevirt.io/vm-delete-protection)\` annotation, filter delete
operations, and will reject any attempt to delete VM objects if the annotation is present and its value is set to “true”
or “True”.
...
| Option     | Deletion time stopped 400 VMs(s) | Deletion time stopped 40 VMs(s) | Deletion time running 40 VMs(s) |
|:-----------|:---------------------------------|:--------------------------------|:--------------------------------|
| Baseline   | 78.97                            | 6.03                            | 198.52                          |
| Option \#1 | 78.96                            | 6.23                            | 199.61                          |
| Option \#2 | 79.06                            | 6.16                            | 199.8                           |

Please note that these tests are not aimed to compare exact deletion times between different experiment sets, but to
compare them to a baseline and check if any option implies a degradation and if there is a faster option than the other.
