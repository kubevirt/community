...
The label `kubevirt.io/volume-update` can be used to filter and identify the VM migrations triggered by a volume update.

The status of the volume update can be monitored by using the label and getting the corresponding VMI migration:
```console
$ kubectl get virtualmachineinstancemigrations -l kubevirt.io/volume-update-migration: vmi` --watch=true
NAME                           PHASE       VMI
