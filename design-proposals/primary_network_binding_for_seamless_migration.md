...
> assign IP addresses from different subnets (one per node).
...
The following proposal attempts to provide a solution from VMs that use
...
- Sticky IP: An IPAM plugin assigns an IP for each pod. In order to
  ...
  target pod.
- Flat Cluster Subnet: In order to allow the same IP to be assigned on a
  different node when migration occurs, only a single subnet needs to
  exist from the entire cluster on the primary pod network.
...
The bridge binding plugin option has been chosen after a successful
initial PoC succeeded.
